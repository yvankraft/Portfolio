...
<h1 id="----scripts----11100">
    <span>Scripts</span>
    <span class="version">@11.10.0</span>
</h1>
...
<li>
<p>Runs on local <code>npm install</code> without package arguments (runs with flags like <code>--production</code> or <code>--omit=dev</code>, but does not run when installing specific packages like <code>npm install express</code>)</p>
</li>
<li>
...
<li>
<p><strong>In workspaces, prepare scripts run concurrently</strong> across all packages. If you have interdependent packages where one must build before another, consider using <code>--foreground-scripts</code> (which can be set in <code>.npmrc</code> with <code>foreground-scripts=true</code>) to run scripts sequentially, or structure your build differently.</p>
</li>
</ul>
...
<p>Use a <code>prepare</code> script to perform build tasks that are platform-independent and need to run before your package is used.
This includes tasks such as:</p>
<ul>
<li>Compiling TypeScript or other source code into JavaScript.</li>
<li>Creating minified versions of JavaScript source code.</li>
<li>Fetching remote resources that your package will use.</li>
</ul>
<p>Running these build tasks in the <code>prepare</code> script ensures they happen once, in a single place, reducing complexity and variability.
Additionally, this means that:</p>
<ul>
<li>You can depend on build tools as <code>devDependencies</code>, and thus your users don't need to have them installed.</li>
...
<p>npm sets the following environment variables from the package.json:</p>
<ul>
<li><code>npm_package_name</code> - The package name</li>
<li><code>npm_package_version</code> - The package version</li>
<li><code>npm_package_bin_*</code> - Each executable defined in the bin field</li>
<li><code>npm_package_engines_*</code> - Each engine defined in the engines field</li>
<li><code>npm_package_config_*</code> - Each config value defined in the config field</li>
<li><code>npm_package_json</code> - The full path to the package.json file</li>
</ul>
<p>Additionally, for install scripts (<code>preinstall</code>, <code>install</code>, <code>postinstall</code>), npm sets these environment variables:</p>
<ul>
<li><code>npm_package_resolved</code> - The resolved URL for the package</li>
<li><code>npm_package_integrity</code> - The integrity hash for the package</li>
<li><code>npm_package_optional</code> - Set to <code>"true"</code> if the package is optional</li>
<li><code>npm_package_dev</code> - Set to <code>"true"</code> if the package is a dev dependency</li>
<li><code>npm_package_peer</code> - Set to <code>"true"</code> if the package is a peer dependency</li>
<li><code>npm_package_dev_optional</code> - Set to <code>"true"</code> if the package is both dev and optional</li>
</ul>
<p>For example, if you had <code>{"name":"foo", "version":"1.2.5"}</code> in your package.json file, then your package scripts would have the <code>npm_package_name</code> environment variable set to "foo", and the <code>npm_package_version</code> set to "1.2.5". You can access these variables in your code with <code>process.env.npm_package_name</code> and <code>process.env.npm_package_version</code>.</p>
<p><strong>Note:</strong> In npm 7 and later, most package.json fields are no longer provided as environment variables. Scripts that need access to other package.json fields should read the package.json file directly. The <code>npm_package_json</code> environment variable provides the path to the file for this purpose.</p>
...
<p>then <code>scripts/build.js</code> will be called for the prepare stage of the lifecycle, and you can check the
<code>npm_lifecycle_event</code> environment variable if your script needs to behave differently in different contexts.</p>
<p>If you want to run build commands, you can do so.
