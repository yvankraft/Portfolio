...
<h3 id="type">type</h3>
<p>The <code>type</code> field defines how Node.js should interpret <code>.js</code> files in your package. This field is not used by npm.</p>
<p>See the <a href="https://nodejs.org/api/packages.html#type">Node.js documentation on the type field</a> for more information.</p>
...
<p><strong>Note on normalization:</strong> When you publish a package, npm normalizes the <code>repository</code> field to the full object format with a <code>url</code> property. If you use a shorthand format (like <code>"npm/example"</code>), you'll see a warning during <code>npm publish</code> indicating that the field was auto-corrected. While the shorthand format currently works, it's recommended to use the full object format in your <code>package.json</code> to avoid warnings and ensure future compatibility:</p>
<pre><code class="language-json">{
  "repository": {
    "type": "git",
...
<p>You can run <code>npm pkg fix</code> to automatically convert shorthand formats to the normalized object format.</p>
...
<h3 id="gypfile">gypfile</h3>
<p>If you have a binding.gyp file in the root of your package and you have not defined your own <code>install</code> or <code>preinstall</code> scripts, npm will default to building your module using node-gyp.</p>
<p>To prevent npm from automatically building your module with node-gyp, set <code>gypfile</code> to <code>false</code>:</p>
<pre><code class="language-json">{
  "gypfile": false
}
</code></pre>
<p>This is useful for packages that include native addons but want to handle the build process differently, or packages that have a binding.gyp file but should not be built as a native addon.</p>
...
<h4 id="replacing-a-dependency-with-a-fork">Replacing a dependency with a fork</h4>
<p>You can replace a package with a different package or fork using several methods:</p>
<p><strong>Using the <code>npm:</code> prefix to replace with a different package name:</strong></p>
<pre><code class="language-json">{
  "overrides": {
...
<p><strong>Using a GitHub repository (supports branches, tags, or commit hashes):</strong></p>
<pre><code class="language-json">{
  "overrides": {
...
<p><strong>Using a local file path:</strong></p>
<pre><code class="language-json">{
  "overrides": {
...
<p>These replacement methods work for both top-level overrides and nested overrides.
For example, to replace a transitive dependency with a fork:</p>
<pre><code class="language-json">{
  "overrides": {
    "parent-package": {
