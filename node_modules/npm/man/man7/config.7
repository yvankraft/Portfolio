.TH "CONFIG" "7" "February 2026" "NPM@11.10.0" ""
.SH "NAME"
...
.P
\fBCommon examples:\fR
.RS 0
.IP \(bu 4
\fBnpm install --prefix /path/to/dir\fR - Runs npm commands in a different directory without changing the current working directory
.IP \(bu 4
\fBnpm install --global\fR - Installs packages globally (shorthand: \fB-g\fR)
.IP \(bu 4
\fBnpm install --save-dev\fR - Saves to devDependencies (shorthand: \fB-D\fR)
.RE 0

.P
Any configuration option documented in the \fI(Config Settings)\fR section below can be set via command line flags using \fB--option-name value\fR syntax.
...
.P
This config cannot be used with: \fBexpect-results\fR
...
.P
When creating a Granular Access Token with \fBnpm token create\fR, this limits the token access to specific organizations.
...
.P
When creating a Granular Access Token with \fBnpm token create\fR, this limits the token access to specific packages.
...
.P
This config cannot be used with: \fBsave-optional\fR, \fBsave-peer\fR, \fBsave-prod\fR
.SS "\fBsave-exact\fR"
.RS 0
.IP \(bu 4
Default: false
...
.P
This config cannot be used with: \fBsave-dev\fR, \fBsave-peer\fR, \fBsave-prod\fR
.SS "\fBsave-peer\fR"
.RS 0
.IP \(bu 4
Default: false
...
.P
This config cannot be used with: \fBsave-dev\fR, \fBsave-optional\fR, \fBsave-prod\fR
.SS "\fBsave-prefix\fR"
.RS 0
.IP \(bu 4
Default: "^"
...
.P
This config cannot be used with: \fBsave-dev\fR, \fBsave-optional\fR, \fBsave-peer\fR
.SS "\fBsbom-format\fR"
.RS 0
.IP \(bu 4
Default: null
...
.P
When creating a Granular Access Token with \fBnpm token create\fR, this limits the token access to specific scopes. Provide a scope name (with or without @ prefix).
