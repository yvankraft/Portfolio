.TH "PACKAGE.JSON" "5" "February 2026" "NPM@11.10.0" ""
.SH "NAME"
...
.IP \(bu 4
Don't put "js" or "node" in the name. It's assumed that it's js, since you're writing a package.json file, and you can specify the engine using the "\fI(engines)\fR" field. (See below.)
.IP \(bu 4
...
.SS "type"
.P
The \fBtype\fR field defines how Node.js should interpret \fB.js\fR files in your package. This field is not used by npm.
.P
See the \fBNode.js documentation on the type field\fR \fI\(lahttps://nodejs.org/api/packages.html#type\(ra\fR for more information.
.SS "browser"
.P
...
.P
Note that you can also set the executable files using \fI(directories.bin)\fR.
.P
...
.RE
.P
\fBNote on normalization:\fR When you publish a package, npm normalizes the \fBrepository\fR field to the full object format with a \fBurl\fR property. If you use a shorthand format (like \fB"npm/example"\fR), you'll see a warning during \fBnpm publish\fR indicating that the field was auto-corrected. While the shorthand format currently works, it's recommended to use the full object format in your \fBpackage.json\fR to avoid warnings and ensure future compatibility:
.P
.RS 2
.nf
{
...
.RE
.P
You can run \fBnpm pkg fix\fR to automatically convert shorthand formats to the normalized object format.
.P
...
.SS "gypfile"
.P
If you have a binding.gyp file in the root of your package and you have not defined your own \fBinstall\fR or \fBpreinstall\fR scripts, npm will default to building your module using node-gyp.
.P
To prevent npm from automatically building your module with node-gyp, set \fBgypfile\fR to \fBfalse\fR:
.P
.RS 2
.nf
{
...
.IP \(bu 4
\fBpath/path/path\fR See \fI(Local Paths)\fR below
.IP \(bu 4
\fBnpm:@scope/pkg@version\fR Custom alias for a package See \fB\[rs]fBpackage-spec\[rs]fR\fR \fI\(la/using-npm/package-spec#aliases\(ra\fR
.RE 0
...
.SS "Local Paths"
.P
As of version 2.0.0 you can provide a path to a local directory that contains a package. Local paths can be saved using \fBnpm install -S\fR or \fBnpm install --save\fR, using any of these forms:
.P
.RS 2
.nf
../foo/bar
...
.RE
.P
This ensures your package \fB@npm/tea-latte\fR can be installed \fIalong\fR with the second major version of the host package \fB@npm/tea\fR only. \fBnpm install tea-latte\fR could possibly yield the following dependency graph:
.P
.RS 2
.nf
├── @npm/tea-latte@1.3.5
...
.RE
.P
we can obtain \fB@npm/awesome-web-framework-1.0.0.tgz\fR file by running \fBnpm pack\fR. This file contains the dependencies \fB@npm/renderized\fR and \fB@npm/super-streams\fR which can be installed in a new project by executing \fBnpm install awesome-web-framework-1.0.0.tgz\fR. Note that the package names do not include any versions, as that information is specified in \fBdependencies\fR.
.P
...
.fi
.RE
.SS "Replacing a dependency with a fork"
.P
You can replace a package with a different package or fork using several methods:
.P
\fBUsing the \fBnpm:\fB prefix to replace with a different package name:\fR
.P
.RS 2
.nf
{
...
.RE
.P
\fBUsing a GitHub repository (supports branches, tags, or commit hashes):\fR
.P
.RS 2
.nf
{
...
.RE
.P
Unless the user has set the \fB\[rs]fBengine-strict\[rs]fR config\fR \fI\(la/using-npm/config#engine-strict\(ra\fR flag, this field is advisory only and will only produce warnings when your package is installed as a dependency.
.SS "os"
.P
...
.RS 0
.P
Note: \fBengines\fR and \fBdevEngines\fR differ in object shape. They also function very differently. \fBengines\fR is designed to alert the user when a dependency uses a different npm or node version than the project it's being used in, whereas \fBdevEngines\fR is used to alert people interacting with the source code of a project.
.RE 0
